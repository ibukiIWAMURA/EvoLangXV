...
# 追加する新しい関数
def calculate_correct_and_failure_ratios(utterances, P):
    correct_count = 0
    failure_count = 0

    for utterance in tqdm(utterances, desc="Calculating Ratios"):
        utt_split = utterance.split(" ")
    ...
    failure_ratio = failure_count / total_samples

    return correct_ratio, failure_ratio
...
# 追加する新しい関数
def plot_correct_ratios(correct_ratios, out_path):
    T = list(range(1, len(correct_ratios) + 1))  # 1からcorrect_ratiosの長さまでの世代
    plt.figure(figsize=(10, 6))
    plt.plot(T, correct_ratios, label='Correct Ratio', marker='o', linestyle='-', color='green')
    plt.title('Correct Ratio Over Generations')
    plt.xlabel('Generation')
    plt.ylabel('Ratio')
    plt.ylim(0, 1)
    plt.xticks(range(1, len(correct_ratios) + 1, 1))
    plt.legend()
    plt.grid(True)
...
if __name__ == '__main__':
    ...
    for i in range(args.n_gens):
        ...
        # 追加：発話が正しく推論された割合を計算
        correct_ratio, _ = calculate_correct_and_failure_ratios(utterances, args.P)
        correct_ratios.append(correct_ratio)
        print(f"Generation {i + 1}: Correct Ratio: {correct_ratio}")

        # （既存のコード）

        # 結果を出力
        with open(f"out/exp{now_fmt}/states_gen_{i + 1}.json", "w") as f:
            settings = {"initial_grammar": args.initial_grammar, "n_gens": args.n_gens,
                        "n_samples": args.n_samples, "p": args.P,
                        "zeros_size": args.zeros_size,
                        "compositional_size": args.compositional_size,
    ...
    # グラフを描画
    plot_correct_ratios(correct_ratios, f"out/exp{now_fmt}/correct_ratios.png")
